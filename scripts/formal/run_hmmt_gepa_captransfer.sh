#!/usr/bin/env bash
# Ours: capability-transfer UCB batch sampling, on HMMT. Config kept identical to
# run_hmmt_gepa.sh except for the sampling strategy.
#
# HMMT is small, so we use ALL of it: train = FlagEval/HMMT_2025 (30, has
# solutions), val = hmmt_feb_2026 (33), test = hmmt_feb_2023 + hmmt_feb_2024 (60).
# Leave the per-split caps at 0 (= full split) to consume every example.
set -euo pipefail
...
# --- Dataset --------------------------------------------------------------
export AIME_DATASET="${AIME_DATASET:-hmmt}"             # registered dataset: aime | math500 | hmmt
# Per-split size caps (0/empty = full split). Keep 0 to use all HMMT data.
export AIME_TRIM_K="${AIME_TRIM_K:-0}"                  # cap for every split
export AIME_TRAIN_K="${AIME_TRAIN_K:-0}"                # train split size (0 = all 30)
export AIME_VAL_K="${AIME_VAL_K:-0}"                    # val split size   (0 = all 33)
export AIME_TEST_K="${AIME_TEST_K:-0}"                  # test split size  (0 = all 60)

# --- Batch sampling strategy ---------------------------------------------
export GEPA_SAMPLING_STRATEGY="capability_transfer"     # baseline (stock GEPA) | capability_transfer
export GEPA_CT_TAU="${GEPA_CT_TAU:-0.5}"                       # solved-threshold on the [0,1] score
export GEPA_CT_ALPHA="${GEPA_CT_ALPHA:-1.0}"                   # Beta prior alpha (prior successes)
export GEPA_CT_BETA="${GEPA_CT_BETA:-1.0}"                     # Beta prior beta (prior failures)
export GEPA_CT_LAMBDA="${GEPA_CT_LAMBDA:-0.2}"                 # UCB exploration weight
export GEPA_CT_COLD_START_BONUS="${GEPA_CT_COLD_START_BONUS:-0.2}"  # bonus for never-sampled batches
export GEPA_CT_USABILITY_WEIGHT="${GEPA_CT_USABILITY_WEIGHT:-1.0}"  # gamma on usability gate (0 disables)
export GEPA_CT_U_ALPHA="${GEPA_CT_U_ALPHA:-1.0}"              # usability Beta prior alpha (prior fixes)
export GEPA_CT_U_BETA="${GEPA_CT_U_BETA:-1.0}"                # usability Beta prior beta (prior non-fixes)

# --- Prompt ---------------------------------------------------------------
# Seed prompt GEPA starts optimizing from (also the baseline eval prompt).
export AIME_INITIAL_PROMPT="${AIME_INITIAL_PROMPT:-Solve the math problem carefully. Break down the steps and provide the final answer.}"
...
export AIME_SOLVER_MAX_TOKENS="${AIME_SOLVER_MAX_TOKENS:-16383}"   # solver max output tokens
...
TEST_ROOT="$REPO_ROOT/examples/aime_math/test/${AIME_DATASET}_formal/gepa_captransfer"
