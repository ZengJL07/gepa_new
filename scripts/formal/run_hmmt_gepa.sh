#!/usr/bin/env bash
# Baseline GEPA training on HMMT. Follows eval_hmmt.sh's config (deepseek-v4-flash
# on api.deepseek.com, CoT off, temperature 0.2, LaTeX answer grading).
#
...
#
# Every configurable knob is listed with its default; override any from the env.
set -euo pipefail
...
# --- Run configuration ----------------------------------------------------
export AIME_MAX_METRIC_CALLS="${AIME_MAX_METRIC_CALLS:-1170}"        # BUDGET: total metric (rollout) calls
